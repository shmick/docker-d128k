...
const ResultsPerPage = 128000

// const PageTemplateHeader = ``
// const PageTemplateFooter = ``
const KeyTemplate = `%s %s %s
`
...
func PageRequest(w http.ResponseWriter, r *http.Request) {
	...
	// Send page header
	 ...
	 // fmt.Fprintf(w, PageTemplateHeader)

	// Send keys
	keys, length := compute(start)
	for i := 0; i < length; i++ {
		key := keys[i]
		fmt.Fprintf(w, KeyTemplate, key.private, key.uncompressed, key.compressed)
	}

	// Send page footer
//	fmt.Fprintf(w, PageTemplateFooter)
}
